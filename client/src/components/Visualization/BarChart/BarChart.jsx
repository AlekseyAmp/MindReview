...
function BarChart ({ data }) {
    ...
    const [chartTitle, setChartTitle] = useState('Анализ настроений по ключевым словам (10 самых популярных)');

    const allWords = Object.values(data).flatMap(sentiment => Object.keys(sentiment));
    const uniqueWords = [...new Set(allWords)].sort((a, b) => {
        const totalOccurrencesA = colorsMap.reduce((acc, { sentiment }) => acc + (data[sentiment]?.[a] || 0), 0);
        const totalOccurrencesB = colorsMap.reduce((acc, { sentiment }) => acc + (data[sentiment]?.[b] || 0), 0);
    ...
    const chartData = topWords.map(word => {
        const values = colorsMap.map(({ sentiment }) => (data[sentiment]?.[word] || 0));
        return [word, ...values];
    });
    ...
    const options = {
        ...
        hAxis: { textStyle: { fontSize: 14, bold: false, fontFace: 'Mulish', color: '#221F1F' } },
        vAxis: {
            ...
            textStyle: {
                fontSize: 12, bold: true, fontFace: 'Mulish', color: '#9C9EA3'
            }
        },
        colors: colorsMap.map(item => item.color),
        bar: { groupWidth: '50%' },
        chartArea: { left: '15%', top: '10%', bottom: '10%', width: 'auto', height: fullChartHeight },
        tooltip: {
            textStyle: {
                color: "#635BFF",
                fontName: "Mulish",
    ...
    return (
        <div className={styles.barChart} style={{ width: "fit-content", height: "fit-content" }}>
            {Object.keys(data).every(key => Object.keys(data[key]).length === 0) ? (
                <div className={styles.notData}>
                    <h3 className={`bold-text`}>{chartTitle}</h3>
                    <p className={`${styles.notFound} dark-text`}>Нет данных для отображения</p>
                </div>
            ) : (
                <>
                    {showLimitedChart && (
                        <Chart
                            ...
                            width="970px"
                            height="650px"
                            data={[['Слово', 'Позитивный', 'Нейтральный', 'Негативный'], ...chartData]}
                            options={options}
                        />
                    )}
                        ...
                        <div className={styles.fullChartOverlay}>
                            <div className={styles.fullChartContainer}>
                                <Chart
                                    ...
                                    data={[['Слово', 'Позитивный', 'Нейтральный', 'Негативный'], ...uniqueWords.map(word => [word, ...(colorsMap.map(({ sentiment }) => (data[sentiment]?.[word]) || 0))])]}
                                    options={options}
                                />
                                <p className={`link-text`} onClick={handleCloseFullChart}>Закрыть ✖</p>
                            </div>
                        </div>
                    )}
                    <div style={{ textAlign: "center", marginTop: "10px" }}>
                        {showLimitedChart && uniqueWords.length > 10 && (
                            <h3 className={`${styles.title} gray-text`} onClick={handleToggleChart}>
