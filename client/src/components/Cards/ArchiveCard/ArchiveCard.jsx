...
function ArchiveCard({ cardKey, analyzeResult }) {
  ...
  const truncatedSourceUrl =
    source_url.length > 15 ? source_url.substring(0, 15) : source_url;

  const maxSentiment = Object.keys(sentiments_data.sentiments).reduce(
    (maxSentiment, currentSentiment) =>
      sentiments_data.sentiments[currentSentiment].count >
      sentiments_data.sentiments[maxSentiment].count
        ? currentSentiment
        : maxSentiment,
    Object.keys(sentiments_data.sentiments)[0]
  );
  const maxSentimentPercentage =
    sentiments_data.sentiments[maxSentiment].percentage;
        ...
        <div className={styles.keywords}>
          <ul>
            ...
            {topKeywords.length === 0 && (
              <li className={`dark-text`}>Нет данных</li>
            )}
          </ul>
        </div>
