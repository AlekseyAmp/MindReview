...
function DragAndDrop({user_id, setIsLoadAnalyze}) {
    ...
    const onDrop = async (acceptedFiles) => {
        ...
        if (file) {
            uploadFile(user_id, file, setError, setShowError, setSuccess, setShowSuccess, setIsLoadAnalyze);
        }
    };
    ...
    return (
        <div className={styles.dropzoneContainer}>
            ...
            <div {...getRootProps()} className={styles.dropzone}>
                <input {...getInputProps()} />
                {isDragActive ? (
                    <p className={`${styles.dropzoneContent} dark-text`}>Перетащите файл сюда...</p>
                ) : (
                    <div className={styles.dropzoneContent}>
                        <img src="../img/icons/drag-and-drop.svg" alt="drag-and-drop" />
                        <p className={`dark-text mt35px`}>Перетащите файл сюда или кликните, чтобы выбрать файл</p>
